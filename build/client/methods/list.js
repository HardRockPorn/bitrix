 ...
 */ export const fillWithCommands = ({ method, params }, start, toProcess, entriesPerCommand)=>{
    ...
    return range(0, requiresCommands).map((i)=>({
            method,
            params: {
                ...params,
                start: start + entriesPerCommand * i
            }
        }));
};
/**
 * Get the highest value from object or an array if there's any
 */ export const highest = (input)=>Object.values(input).reduce((a, b)=>a !== undefined && b !== undefined ? a > b ? a : b : a !== undefined && b === undefined ? a : a === undefined && b !== undefined ? b : undefined, undefined);
/**
 ...
 */ export const batchToListPayload = (payload)=>{
    ...
    const flattenResult = Object.entries(result).reduce(// @ts-expect-error ignore this for now
    (flatten, [_key, r])=>!r ? flatten : [
            ...flatten,
            ...r
        ], []);
    return {
 ...
 */ export default (({ call, batch })=>{
    const list = async (method, params)=>{
        ...
        const listAll = async ()=>{
            const batchCommands = fillWithCommands({
                method,
                params
            }, start, firstCall.total, MAX_ENTRIES_PER_COMMAND);
            const payload = await batch(batchCommands);
            // @ts-expect-error ignore this for now
            return batchToListPayload(payload);
        };
        const firstCall = await call(method, {
            ...params,
            start
        });
