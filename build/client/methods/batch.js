 ...
 */ export const chunkCommands = (commands, size = MAX_COMMANDS_PER_BATCH)=>{
    ...
    return chunks.map((c)=>commandsIsArray ? c.map(([_key, command])=>command) : fromPairs(c));
};
/**
 ...
 */ export default (({ get })=>{
    const batch = async (commands, commandsPerRequest = MAX_COMMANDS_PER_BATCH)=>{
        const call = (c)=>get(Method.BATCH, {
                searchParams: prepareCommandsQueries(c)
            }).then(({ body })=>body);
        const calls = chunkCommands(commands, commandsPerRequest).map(call);
        return await Promise.all(calls).then((chunkedPayloads)=>{
